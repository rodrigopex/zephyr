...
* Choose the subscriber notification queue size carefully to accommodate the notifications.


Message delivery sequence
-------------------------

The listeners (synchronous observers) will follow the channel definition sequence as the notification and message consumption sequence. However, the subscribers, as they have an asynchronous nature, all will receive the notification as the channel definition sequence but only will consume the data when they execute again, so the delivery respects the order, but the reaction of it will be determined by the priority assigned to the subscribers.

Implementation
**************

Zbus operation depends on channels and observers. Therefore, it is necessary to determine its message and observers list during the channel definition. A message is a regular C struct; the observer can be a subscriber (asynchronous) or a listener (synchronous). Channels can have a combination of the following characteristics:

* ``Validator function`` makes a channel accepts only valid messages;
* ``Read-only flag`` makes a channel read-only.

.. warning::

    The ``Validator function`` will not take effect in read-only channels.

The following code defines and initializes a read-only channel and its dependencies. This channel represents a firmware version, for example.
...
.. code-block:: c
    ...
    {
            LOG_DBG("%d - Channel %s:", count, chan->name);
            LOG_DBG("      Message size: %d", chan->message_size);
            ++count;
            LOG_DBG("      Observers:");
    ...
    {
            LOG_DBG("%d - %s %s", count, ((obs->queue != NULL) ? "Subscriber" : "Listener"), obs->name);
            ++count;
            return true;
    }
    void main(void)
...
There is a possibility of injecting data into the channel's metadata by setting the :kconfig:option:`CONFIG_ZBUS_CHANNEL_USER_DATA_SIZE`. Thus, ``user data`` enables in compile time a memory area of :kconfig:option:`CONFIG_ZBUS_CHANNEL_USER_DATA_SIZE` size into all channels metadata and make this accessible by the ``user_data`` field.
...
.. code-block:: c
    ...
    if (!zbus_chan_claim(&acc_chan, K_NO_WAIT)) {
...
It is possible to add observers to channels in runtime. This feature uses the object pool pattern technique which the dynamic nodes are pre allocated and can be used and recycled. For enabling that it is necessary to set the size of the pool by changing the feature :kconfig:option:`CONFIG_ZBUS_RUNTIME_OBSERVERS_POOL_SIZE`. The following example illustrates that.
...
Zbus can only use a limited number of dynamic observers. The configuration option :kconfig:option:`CONFIG_ZBUS_RUNTIME_OBSERVERS_POOL_SIZE` represents the size of the runtime observers pool. Change that to fit the solution needs. Use the :c:func:`zbus_runtime_obs_pool_available` to verify how many available runtime observers slots are available. The function :c:func:`zbus_runtime_obs_pool_watermark` will provide information regarding the maximum number of used slots count reached during the execution. Use that to set the appropriate pool size avoiding waste.
...
* :kconfig:option:`ZBUS_CHANNEL_NAME`
* :kconfig:option:`ZBUS_OBSERVER_NAME`
* :kconfig:option:`CONFIG_ZBUS_RUNTIME_OBSERVERS_POOL_SIZE`
* :kconfig:option:`CONFIG_ZBUS_CHANNEL_USER_DATA_SIZE`
* :kconfig:option:`CONFIG_ZBUS_ASSERT_MOCK`
