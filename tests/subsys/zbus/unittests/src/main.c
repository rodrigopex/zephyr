...
bool hard_msg_validator(const void *msg, size_t msg_size)
{
	ARG_UNUSED(msg_size);
...
static bool always_true_chan_iterator(struct zbus_channel *chan)
{
	return true;
}

static bool always_true_obs_iterator(struct zbus_observer *obs)
{
	return true;
}

static bool always_false_chan_iterator(struct zbus_channel *chan)
{
	return false;
}

static bool always_false_obs_iterator(struct zbus_observer *obs)
{
	return false;
}

static bool check_chan_iterator(struct zbus_channel *chan)
{
	static int chan_idx;
...
static bool check_obs_iterator(struct zbus_observer *obs)
{
	static int obs_idx;
...
static bool count_false_chan_iterator(struct zbus_channel *chan)
{
	++idx;
...
static bool count_false_obs_iterator(struct zbus_observer *obs)
{
	++idx;
...
ZTEST(basic, test_readonly_channel)
{
	zassert_equal_ptr(NULL, zbus_chan_msg(&version_chan), NULL);
	const void *msg = version_chan.message;

	zassert_equal_ptr(msg, zbus_chan_const_msg(&version_chan), NULL);
	...
	struct version_msg ver;

	zbus_chan_read(&version_chan, &ver, K_NO_WAIT);
	zassert_equal(ver.major, 0, "Major must be 0");
	zassert_equal(ver.minor, 1, "Minor must be 1");
	zassert_equal(ver.build, 1023, "Build must be 1023");

	zassert_equal(version_chan.read_only, true, "It must be true");

	zassert_equal(version_chan.message_size, sizeof(struct version_msg),
		      "Message size is wrong");

	zassert_equal(version_chan.read_only, true, NULL);

	zassert_equal(-EPERM, zbus_chan_pub(&version_chan, &ver, K_MSEC(200)), NULL);

	zassert_equal(-EPERM, zbus_chan_add_obs(&version_chan, &fast_lis, K_MSEC(200)), NULL);

	zassert_equal(-EPERM, zbus_chan_notify(&version_chan, K_NO_WAIT), NULL);

	zassert_equal(-EPERM, zbus_chan_rm_obs(&version_chan, &fast_lis, K_MSEC(200)), NULL);

	zassert_is_null(version_chan.observers[0], "There is no subscriber, it must be NULL");
}
